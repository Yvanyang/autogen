"""CodeBERT-based agent for code repository vectorization and querying."""
...
from autogen_core import Component, ComponentBase, ComponentFromConfig, ComponentLoader, ComponentSchemaType, ComponentToConfig
...
class CodeBERTRepositoryAgentConfig(BaseModel):
    """Configuration for the CodeBERT repository agent."""
    
    name: str = Field(default="codebert_repository_agent", description="Name of the agent")
    ...
    code_repo_memory: Dict[str, Any] = Field(default={}, description="Additional configuration for CodeBERTRepositoryMemory")
    description: str = Field(
        default="A specialized agent for code repository understanding and question answering using CodeBERT",
        description="Description of the agent",
    )

class CodeBERTRepositoryAgent(ComponentBase, ComponentFromConfig, ComponentToConfig, ComponentSchemaType, ComponentLoader):
    """Agent for code repository vectorization and querying using CodeBERT.
    
    This agent provides functionality to vectorize and query code repositories using
    Microsoft's CodeBERT model. It can accept a repository URL or path, check if it's already 
    vectorized, process it if not, and then enable question-answering functionality.
    
    Args:
        config (CodeBERTRepositoryAgentConfig): Configuration for the agent
        
    Example:
        ```python
        import asyncio
        from autogen_ext.agents.code_repository.codebert_agent import CodeBERTRepositoryAgent, CodeBERTRepositoryAgentConfig
        ...
        async def main():
            ...
            agent = CodeBERTRepositoryAgent(
                config=CodeBERTRepositoryAgentConfig(
                    name="repo_agent",
    ...
    component_type = "codebert_repository_agent"
    component_config_schema = CodeBERTRepositoryAgentConfig
    
    def __init__(self, config: CodeBERTRepositoryAgentConfig) -> None:
        """Initialize CodeBERTRepositoryAgent."""
        self._config = config
    ...
    async def prepare(self) -> bool:
        ...
        if self._code_repo_memory is None:
            memory_config = CodeBERTRepositoryMemoryConfig(
            ...
            self._code_repo_memory = CodeBERTRepositoryMemory(config=memory_config)
        ...
        if self._assistant_agent is None:
            model_client = self.load_component(self._config.model_client)
    ...
    async def run(self, task: str) -> str:
        ...
        if not self._assistant_agent:
            raise RuntimeError("Failed to create assistant agent during preparation")
    ...
    def _to_config(self) -> CodeBERTRepositoryAgentConfig:
        """Serialize the agent configuration."""
        return self._config
    
    @classmethod
    def _from_config(cls, config: CodeBERTRepositoryAgentConfig) -> Self:
